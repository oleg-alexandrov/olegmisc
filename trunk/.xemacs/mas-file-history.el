...
(defun oleg3 ()
  "Save the histories from `oleg-savehist-history-variables' to `oleg-savehist-file'.
A variable will be saved if it is bound and non-nil."
  ...
  (save-excursion
    ...
    (let ((find-file-hooks nil)
	  (buffer-exists-p (get-file-buffer "Xhistory.el")))
      (set-buffer (find-file-noselect "Xhistory.el"))
      (unwind-protect
	  (progn
	    (erase-buffer)
	    (let ((print-length nil)
		  ...
		  (print-level nil)
		  (print-readably t)
		  (sym     file-name-history) )
	      (dolist (xuxa file-name-history)
		  (prin1
		   xuxa
		   (current-buffer))
		  (insert "\n")
