...
int main( int argc, char**argv ){
  ...
  if (argc < 4){
    std::cerr << "Usage: " << argv[0] << " input.txt utm output.txt" << std::endl;
    exit(0);
  }
  ...
  while( getline(file, line, '\n') ) {
    ...
    Vector2 ll = geo.point_to_lonlat(Vector2(x, y));
    outfile2 << ll[0] << ' ' << ll[1] << ' ' << h << std::endl;
  }
  outfile2.close();
  ...
  read_georeference( dem_georef, input_name );
  ...
  vw_out() << "2Calculating the number of points in the file!" << std::endl;
  {
	...
	Vector3 lonlatrad( lonlat.x(), lonlat.y(), dem(i,j) );
        ...
        if ( xyz != Vector3() && xyz == xyz ) {
          outfile2 << lonlat[0] << ' ' << lonlat[1] << ' ' << dem(i, j) << std::endl;
          point_cloud(i, j) = xyz;
          local_mean += xyz;
          local_count++;
        }
