...
for s in Types:
    ...
    #opt_ctr_lat_lon_h2 = np.array([cam_lat_lon_h_from_tsai(cam) for cam in opt_cams2])
    ...
    #opt_h2 = [r[2] for r in opt_ctr_lat_lon_h2]
    #opt_lon2 = [r[1] for r in opt_ctr_lat_lon_h2]
    #opt_lat2 = [r[0] for r in opt_ctr_lat_lon_h2]
    ...
    if subtractLineFit:
        ...
        #opt_lat2 = opt_lat2 - fit_lat
        #opt_lon2 = opt_lon2 - fit_lon
        #opt_h2 = opt_h2 - fit_h

        residualTag = ' residual'
        
    #ax[1].scatter(np.arange(len(orig_lon)), orig_lon, label='orig', s=1)
    # Plot residuals after subtracting a linear fit
    ax[count][0].plot(np.arange(len(orig_lat)), orig_lat, label='orig', color = 'b')
    ax[count][0].plot(np.arange(len(opt_lat)), opt_lat, label=baDir, color = 'r')
    #ax[count][0].plot(np.arange(len(opt_lat)), opt_lat2, label='opt_triWt0.02_transWt0.2', color = 'g')

    ax[count][1].plot(np.arange(len(orig_lon)), orig_lon, label='orig', color = 'b')
    ax[count][1].plot(np.arange(len(opt_lon)), opt_lon, label=baDir, color = 'r')
    #ax[count][1].plot(np.arange(len(opt_lon)), opt_lon2, label='opt_triWt0.02_transWt0.2', color = 'g')

    ax[count][2].plot(np.arange(len(orig_h)), orig_h, label='orig', color = 'b')
    ax[count][2].plot(np.arange(len(opt_h)), opt_h, label=baDir, color = 'r')
    #ax[count][2].plot(np.arange(len(opt_h)), opt_h2, label='opt_triWt0.02_transWt0.2', color = 'g')
