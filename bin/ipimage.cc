...
int main(int argc, char** argv) {
  ...
  if( input_file_names.size() < 3 ) {
    vw_out() << "Error: Must specify at least two input files and prefix!" << std::endl << std::endl;
    vw_out() << usage.str();
    return 1;
  }

  std::vector<InterestPoint> final_ip1, final_ip2;
  std::string left_input_file = input_file_names[0];
  std::string right_input_file = input_file_names[1];
  std::string out_prefix = input_file_names[2];
  
  std::string match_filename
    = ip::match_filename(out_prefix, left_input_file, right_input_file);

  vw_out() << "Reading: " << match_filename << std::endl;
  ip::read_binary_match_file(match_filename, final_ip1, final_ip2 );
  vw_out() << "Read points: " << final_ip1.size() << ' ' << final_ip2.size()
           << std::endl;
  
  std::string match_image = out_prefix + "-matches.tif";
  vw_out() << "Writing: " << match_image << std::endl;
  write_match_image(match_image,
