...
// center, intersect the given dem (via interpolation) and compute the
// resulting distance from the starting point to the intersection
// point.
...
int main( int argc, char *argv[] ){
  ...
  try {
    ...
    for (int i = 0; i < (int)llh.size(); i++){
      ...
      errors.push_back(std::abs(p[2] - v.child()));
    }
    ...
    double p16 = errors[std::min(len-1, (int)round(len*0.16))];
    double p50 = errors[std::min(len-1, (int)round(len*0.50))];
    double p84 = errors[std::min(len-1, (int)round(len*0.84))];
    vw_out() << "Error percentiles: "
             << " 16%: " << p16 << ", 50%: " << p50 << ", 84%: " << p84 << endl;
    
    double mean = calc_mean(errors, len);
