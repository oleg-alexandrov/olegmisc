...
int main( int argc, char**argv ){
  ...
  double scale = 1;
  Vector2 focal = P.focal_length();
  Vector2 offset = P.point_offset();
  focal *= scale;
  offset *= scale;
  boost::shared_ptr<LensDistortion> lens = P.lens_distortion()->copy();
  lens->scale( scale );
  Vector3 ctr = P.camera_center();
  double r = 6378137;
  double extra = 170000;

  ctr = (r+extra)*ctr/norm_2(ctr);

  PinholeModel2 P2( ctr,
                    P.camera_pose().rotation_matrix(),
                    focal[0], focal[1], offset[0], offset[1],
                    P.coordinate_frame_u_direction(),
                    P.coordinate_frame_v_direction(),
                    P.coordinate_frame_w_direction(),
                    *lens
                    );

  std::cout << "---scaled: " << P2 << std::endl;
