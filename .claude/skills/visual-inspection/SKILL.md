...
**No text inside a figure that ships with an RST/HTML caption - the caption below carries almost ALL of it.** No panel titles ("before"/"after"/"hillshade"), no baked-in descriptions. The prose caption names the panels left-to-right and states the units and clamp. The ONE allowed exception is the colorbar: KEEP its tick numbers AND a short unit label - that label is fine and helpful (Oleg 2026-09-04, revising the 2026-08-20 stricter rule that had banned it). SPELL OUT the unit word ("elevation in meters", "meters", "pixels"), never a bare single letter like "m" (at figure scale a lone "m" reads like a sideways epsilon). Everything else stays out of the image. (Oleg 2026-08-20.)

**Size the in-figure text (colorbar label + ticks) LARGE - the PNG is downscaled in the doc.** An RST `.. figure::` renders at maybe 50-60% of column width, so matplotlib's default label/tick sizes shrink to illegible - the colorbar text ends up far smaller than the caption prose right below it, which looks broken. There is no autoscale that fixes this; you must set it. Use a generous colorbar label fontsize (~20-24) and tick labelsize (~18-20) so the text is still readable after the doc shrinks the image. Before finalizing, look at the rendered figure at its doc size (open the built HTML/PDF, or scale the PNG to the figure width) and confirm the colorbar text reads at roughly the caption's size; if it is visibly smaller, bump the fonts and re-render. (Oleg 2026-09-04.)
